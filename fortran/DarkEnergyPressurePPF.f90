    ...
    contains
    procedure :: ReadParams => TDarkEnergyPressurePPF_ReadParams
    procedure :: Init => TDarkEnergyPressurePPF_Init
    procedure :: PerturbedStressEnergy => TDarkEnergyPressurePPF_PerturbedStressEnergy
    procedure :: diff_rhopi_Add_Term => TDarkEnergyPressurePPF_diff_rhopi_Add_Term
    procedure, nopass :: PythonClass => TDarkEnergyPressurePPF_PythonClass
    procedure, nopass :: SelfPointer => TDarkEnergyPressurePPF_SelfPointer
    procedure, private :: setcgammappf
    end type TDarkEnergyPressurePPF

    public TDarkEnergyPressurePPF
    contains

    subroutine TDarkEnergyPressurePPF_ReadParams(this, Ini)
    use IniObjects
    class(TDarkEnergyPressurePPF) :: this
    class(TIniFile), intent(in) :: Ini

    call this%TDarkEnergyDensityAndPressure%ReadParams(Ini)
    ...
    end subroutine TDarkEnergyPressurePPF_ReadParams
    ...
    subroutine TDarkEnergyPressurePPF_SelfPointer(cptr, P)
    use iso_c_binding
    Type(c_ptr) :: cptr
    Type (TDarkEnergyPressurePPF), pointer :: PType
    class (TPythonInterfacedClass), pointer :: P

    call c_f_pointer(cptr, PType)
    P => PType

    end subroutine TDarkEnergyPressurePPF_SelfPointer

    subroutine TDarkEnergyPressurePPF_Init(this, State)
    use classes
    use config
    class(TDarkEnergyPressurePPF), intent(inout) :: this
    class(TCAMBdata), intent(in), target :: State
    call this%TDarkEnergyDensityAndPressure%Init(State)
    ...
    class(TDarkEnergyPressurePPF), intent(in) :: this
    real(dl), intent(in) :: dgrhoe, dgqe, grho, gpres, w, Pgrhova2, grhok, adotoa, &
    ...
    if (this%is_cosmological_constant .or. this%no_perturbations) then
        ...
        ppiedot = 3._dl * dgrhoe + dgqe * &
            ...
            (grhov_t + Pgrhova2) * k * z / adotoa - 2._dl * k2 * Kf1 * &
    ...
    S_Gamma = (grhov_t  + Pgrhova2) *(vT + sigma) * k / adotoa / 2._dl / k2
    ...
    dgqe = -dgqe / Fa * 2._dl * k * adotoa + vT * (grhov_t + Pgrhova2)
    dgrhoe = -2 * k2 * kf1 * Gamma - 3 / k * adotoa * dgqe

    end subroutine TDarkEnergyPressurePPF_PerturbedStressEnergy

    end module DarkEnergyPressurePPF
